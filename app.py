...
start_date,end_date = st.sidebar.date_input(label='Rentang Waktu', min_value=min_date,max_value=max_date,value=[min_date,max_date])

#input kategori
categories = ["All categories"] + list(data['category'].value_counts().keys().sort_values())
category= st.sidebar.selectbox(label='kategory',options=categories)
...
# visualisaso dengan bar chart
st.header(':video_camera: Channel')
bar_data = outputs['channel_name'].value_counts().nlargest(10)
fig=px.bar(bar_data, color=bar_data,orientation='h',title=f'Chanel Terpopuler Dari Kategori {category}')
st.plotly_chart(fig)
